...
class Settings(BaseSettings):
    ...
    # То же для slug'а предмета (title_en из ML-сервиса) против нашего label_slug
    # — сравниваются канонические ключи после свёртки транслитерации (см.
    # recognizer.normalize_slug), так что нечёткая ветка добирает только мелочи.
    # Порог выше, чем у названий: slug'и короткие, а соседние предметы музея
    # отличаются в них одним-двумя словами. 0 отключает нечёткое сопоставление.
    recognition_slug_match_cutoff: float = 0.9
    ...
    # Предел длины одного запроса к SpeechKit v3, знаков. Замер 02.09.2026: 249
    # синтезируется, 250 — отказ, поэтому длинный текст режется на куски
    # (app/services/tts.split_for_v3). Настройка нужна на случай, если предел
    # сервиса изменится: менять придётся число, а не код.
    speechkit_v3_max_chars: int = 249
